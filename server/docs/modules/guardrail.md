...
for the agent, and the guardrail strips it the same way both times, so the judge sees the same
text it judged the first time. Which line a trip gets is keyed on
